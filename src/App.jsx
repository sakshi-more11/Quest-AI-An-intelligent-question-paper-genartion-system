...
export default function App() {
  ...
  const renderPage = () => {
    ...
    switch (activeTab) {
      ...
      case "history":
        return <ActivityLogs user={user} logs={logs} />;
...
// ── Fallback for access-denied routes ─────────────────────────────────────────
function AccessDenied() {
  return (
    <div className="flex items-center justify-center h-64">
      <div className="text-center">
        <div className="text-4xl mb-3">🚫</div>
        <h3 className="text-lg font-semibold mb-1" style={{ color: "#EF4444" }}>Access Denied</h3>
        <p className="text-sm" style={{ color: "#64748B" }}>This section is not available for your role.</p>
      </div>
    </div>
  );
}
